 ...
 * @author Jörg Meier
 */
public class HttpProxy extends MediaProxyPlugin {

	private static Logger log = null;
	private static SettingProxy sproxy = null;

	static {
		log = Logger.getLogger(HttpProxy.class);
		
		try {
			sproxy = SettingProxy.getInstance(HttpProxy.class);		
			Setting mySetting;
			...
			HttpProxy.log.error("Error registering setting.");
		}
	}
	...
	private static int END_PORT = 0;
	/** the next port to use */
	private static int currentPort = 0;
	private static int numberOfRetry = 0;
	...
	protected URL initPlugIn() throws MediaProxyException {
		...
		try {
			...
			all_ips = InetAddress.getAllByName(host); 
			log.error("IP of given hostname: " + ip.toString());
			
			for(int j=0;j<all_ips.length;j++){
				log.error("All IPs :" + all_ips[j]);
			}
